...
`corpora.json` selects the corpus version/config and its Parquet path. All fields are loaded from the current dataset, with a 60-second server refresh cache and no commit pin. Keep prior entries for historical browsing. Scoring currently supports v1; introducing another scorer requires preserving an executable scorer implementation per historical scorer version before deployment. Do not silently reinterpret historical results with new arithmetic. Metrics content is versioned in source alongside that scorer. Alignment, waqf segmentation, and word timing have separate scorers and leaderboard views.
