<script lang="ts">
  ...
  const columnHelp: Record<string, string> = {
    ...
    rtf: 'Processing time relative to audio length. 0.1 means one minute of audio took six seconds. Lower is faster; times are self-reported and depend on hardware.',
  ...
  const alignmentCols = [{key:'words_f1', label:'Words F1'}, {key:'clean_segments', label:'Clean segments'}, {key:'repeats_f1', label:'Repeats F1'}, {key:'words_per_segment', label:'Words / segment'}, {key:'rtf', label:'RTF'}, {key:'confidence_skill', label:'Confidence skill'}];
  ...
  $: baseCols = task==='alignment'?alignmentCols:task==='segmentation'?[{key:'segments_f1',label:'Segments F1'},{key:'boundaries_f1',label:'Boundaries F1'},{key:'rtf',label:'RTF'}]:[{key:'words_timed',label:'Words timed correctly'},{key:'clean_clips',label:'Clean clips'}];
  ...
  function value(r: Row,k: string) {return k==='system'?(r.display_name||r.system):k in r?r[k]:r.scores[k];}
  ...
  function fmt(k: string,v: any) {if(v==null)return '—'; if(k==='words_per_segment')return v.toFixed(1); if(k==='rtf')return v.toFixed(3); if(k==='confidence_skill')return v.toFixed(3); return `${(v*100).toFixed(1)}%`;}
  function sort(k: string) {ascending=sortKey===k?!ascending:k==='rank'||k==='system'||k==='rtf'||k==='user_parameters';sortKey=k;}
...
{:else}
  ...
  {#if batchPreview}<section class="preview"><h2>Review all selected tasks</h2>
    <div class="batch-results">{#each selectedTasks as key}{@const result=batchPreview.tasks[key]}
      ...
      {#if result.scores}<div class="preview-scores">{#each Object.entries(result.scores).filter(([metric])=>['words_f1','clean_segments','repeats_f1','segments_f1','boundaries_f1','rtf','words_timed','clean_clips'].includes(metric)) as [metric,value]}<div><span>{({words_f1:'Words F1',clean_segments:'Clean segments',repeats_f1:'Repeats F1',segments_f1:'Segments F1',boundaries_f1:'Boundaries F1',rtf:'RTF',words_timed:'Words timed correctly',clean_clips:'Clean clips'} as Record<string,string>)[metric]}</span><strong>{fmt(metric,value)}</strong></div>{/each}</div>{/if}
...
<dialog bind:this={dialog} aria-labelledby="system-title"><div class="dialog-heading"><h2 id="system-title">{detail?.display_name||detail?.system}</h2><button class="close" aria-label="Close system details" onclick={()=>dialog.close()}>×</button></div>{#if detail}{#if detail.synthetic}<p class="synthetic-label">Synthetic example generated from the reference annotations for UI testing. Not a real system result.</p>{/if}<p class="system-description">{detail.description}</p><a href={detail.url} target="_blank" rel="noreferrer">Website / repository ↗</a><dl class="system-facts"><dt>User-facing parameters</dt><dd>{detail.user_parameters}</dd>{#if detail.hardware}<dt>Hardware</dt><dd>{detail.hardware} ({detail.hardware_class?.toUpperCase()})</dd>{/if}<dt>Last submitted</dt><dd>{new Date(detail.submitted_at).toLocaleDateString()}</dd><dt>Corpus</dt><dd>{corpus?.version}</dd></dl>{/if}</dialog>
